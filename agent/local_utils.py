...
def ensure_formula_object(x):
    out = {"top": x.get("top", []), "mid": x.get("mid", []), "base": x.get("base", [])}
    out["total_grams"] = float(x.get("total_grams", sum(i.get("grams", 0.0) for r in ("top","mid","base") for i in out[r])))
    return out
